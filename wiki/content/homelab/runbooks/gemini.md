...
3.  **Bulk Backup & Archive Storage**
    *   **Physical:** 5TB Seagate HDD.
    *   **Mount Point:** `/mnt/misc_5tb`.
    *   **Usage:** Kopia repository, Docker migration archives, exports, documents, media imports, and retired service data.
    *   **Rule:** Use purpose folders under `/mnt/misc_5tb` such as `backups/`, `exports/`, `documents/`, `media-imports/`, `service-data/`, and `retired/`.

4.  **Fast Staging SSD**
    *   **Physical:** 2TB Samsung NVMe.
    *   **Mount Point:** `/data`.
    *   **Usage:** High-speed staging, scratch, imports, incomplete downloads, and future explicitly selected fast app/database data.
    *   **Rule:** Staging paths live under `/data/staging`; durable fast paths must be intentionally chosen before use.
...
*   **Source of truth:** Git remains authoritative for homelab documentation.
*   **Wiki UI:** The `wiki/` stack runs Wiki.js and renders generated content
    from `wiki/content/`.
*   **Sync commands:** Use `./scripts/wiki-sync.sh --check`,
    `./scripts/wiki-sync.sh --backfill`, and
    `./scripts/wiki-sync.sh --stack <stack>`.
*   **Agent rule:** Do not treat manual Wiki.js edits as durable. Update Git
    docs or generated wiki source, then publish.
*   **MCP:** Wiki.js MCP access is optional and depends on a Wiki.js API token;
    repo scripts remain the authoritative update path.
...
*   **Internal Routing:**
    ...
    *   **Reverse Proxy:** Traefik is the target-state IaC reverse proxy.
        Nginx Proxy Manager may remain live during transition and rollback.
    *   **VPN Routing:** The `arr-suite` uses `gluetun` as a VPN gateway. Services like `qbittorrent` and `prowlarr` use `network_mode: service:gluetun` to route traffic through the VPN.
