...
* **Infrastructure Source of Truth:** Git-tracked Compose, SOPS env files, scripts, workflows, and generated wiki content under `/home/ethan/docker`.
* **Wiki Source of Truth:** Wiki.js is a UI mirror. Persistent documentation changes must be made in Git and synced with `./scripts/wiki-sync.sh`.
...
* **Bulk Backup & Archive Storage:** 5TB HDD.
    * **Usage:** Local backups, Kopia repository, exports, documents, media imports, service-data archives, and retired data.
    * **Mount Point:** `/mnt/misc_5tb`
    * **Path Standard:** Use purpose folders such as `backups/`, `exports/`, `documents/`, `media-imports/`, `service-data/`, and `retired/`.
* **Fast Staging SSD:** 2TB NVMe.
    * **Usage:** High-speed staging, scratch, incomplete downloads, imports, and future explicitly selected fast app/database data.
    * **Mount Point:** `/data`
    * **Path Standard:** Staging lives under `/data/staging`; durable fast data lives under `/data/fast-data` only after an explicit migration.
...
* **Reverse Proxy:** Traefik is the target-state IaC reverse proxy. Nginx Proxy Manager is legacy/rollback until cutover is fully verified.
...
### D. Wiki and Agent Context
* **Wiki.js:** The `wiki/` stack exposes `wiki.ethan-herring.com`,
  `wiki.pup-percy.com`, and `wiki.ethanh.online`.
* **Focus Stack:** When working on a stack, update/regenerate
  `/homelab/stacks/<stack>` with `./scripts/wiki-sync.sh --stack <stack>`.
* **Backfill:** Use `./scripts/wiki-sync.sh --backfill` to refresh current docs
  plus safe historical context from Git/docs/rollout summaries.
* **MCP:** Existing Wiki.js MCP servers may be used after API token creation, but
  Codex should prefer repo scripts for authoritative updates.
...
* **`arr-suite/`**:
    * **Gateway:** `gluetun` (VPN).
    * **Dependency:** `qbittorrent`, `prowlarr` use `network_mode: service:gluetun`.
...
* **Database Corruption:** Check `/home/ethan/docker` backups under `/mnt/misc_5tb/backups`.
